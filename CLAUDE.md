...
- 2026-04-16 · HTML entities decoded once at OG extraction, not downstream · regex attribute extraction captured raw `&#x27;`/`&amp;`; front-end `escHtml` then double-escaped, producing literal `&#x27;` on cards. Also cleaned up `&amp;` inside og:image query strings that were breaking CDN image fetches
- 2026-04-16 · Rule-based text enrichment runs synchronously at capture time via `api/_lib/enrich-rules.js` · ports the TOOL/STYLE/MOOD/LOCATION tables + platform-aware format map + subject keyword mining from the archived Python. 2-tag items → 6-15 tag items; no network calls beyond the single OG fetch
- 2026-04-16 · `enrichment_status` column (`pending|text_done|vision_done|error`) drives both progressive capture UX and the login-time re-enrich filter · distinguishes "text rules ran" from "vision done", lets client poll for live tag updates and backfill precisely what's stale
- 2026-04-16 · Capture returns after text rules; vision runs async, client polls Supabase directly every 3s (up to 30s) · no Vercel-specific `waitUntil`, works on any infra. `PanelManager.refreshItem(slug)` live-updates an open panel's body+footer without re-mounting
- 2026-04-16 · Storage INSERT **and** UPDATE policies both required for `upsert:true` uploads · original schema.sql left them as a dashboard-copy TODO comment; UPDATE half was easy to miss and caused silent RLS-violation "new row violates row-level security policy" on every re-upload. Now embedded as executable SQL in schema.sql
- 2026-04-16 · `scripts/local-dev.js` (zero-dep, ~135 lines) is the default local dev entry · `vercel dev` requires a Vercel login even for localhost, which is a speed bump for anyone cloning. Routes `/api/:path` to `api/:path.js`, reuses `authenticateRequest` so auth isn't bypassed (the issue that got the old `server.js` deleted)
- 2026-04-16 · Pre-fix library items auto-heal on next login via `/api/reprocess` + silent client drip · preserves user-assigned intent tags + prior vision tags, retries retryable upload failures on subsequent logins, terminally marks no-image items `vision_done` so backfill doesn't loop
